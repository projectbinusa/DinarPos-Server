...
@Service
public class ExcelKabKot {
    @Autowired
    private KabKotRepository kabKotRepository;

    public static String TYPE = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet";

    static String[] HEADERsTemplate = {"NO", "NAMA PROVINSI" , "NAMA KABUPATEN / KOTA" };
    ...
    public List<KabKot> excelToKabKot(InputStream is) {
        ...
        try {
            ...
            List<KabKot> kabkotList = new ArrayList<KabKot>();
            int rowNumber = 0;
            while (rows.hasNext()) {
                ...
                while (cellsInRow.hasNext()) {
                    ...
                    switch (cellIdx) {
                        case 2:
                            kabkot.setNama_kabkot(currentCell.getStringCellValue());
                            break;
                        case 1:
    ...
    public void saveBarang(MultipartFile file) {
        try {
            List<KabKot> kabKot = excelToKabKot(file.getInputStream());
            kabKotRepository.saveAll(kabKot);
        } catch (IOException e) {
            throw new RuntimeException("fail to store excel data: " + e.getMessage());
        }
    }
    public ByteArrayInputStream templateKabKot() throws IOException {
