...
public class ExcelCustomerCP {
    ...
    public static ByteArrayInputStream customerGoogleToExcel(List<CustomerCP> customers) throws IOException {
        try (Workbook workbook = new XSSFWorkbook(); ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            Sheet sheet = workbook.createSheet(SHEET);
            ...
            for (CustomerCP customer : customers) {
                Row row = sheet.createRow(rowIdx++);
                row.createCell(0).setCellValue(no);
                row.createCell(2).setCellValue(customer.getNama_cp());
                row.createCell(3).setCellValue(customer.getCustomer().getNama_customer());
                row.createCell(33).setCellValue(customer.getNo_hp());
                no++;
            }
            workbook.write(out);
            return new ByteArrayInputStream(out.toByteArray());
